use rustix::fs::{open, ioctl_blkpbszget, ioctl_blksszget, OFlags, Mode, statx, AtFlags, StatxFlags};
...
pub fn fetch<P: AsRef<Path>>(path: P) -> io::Result<DirectInfo> {
  ...
  match ioctl_blksszget(&fd) {
    ...
    Err(_) => {
      ...
      let stx = statx(parent_fd, file_name, AtFlags::empty(), StatxFlags::BASIC_STATS)?;
      ...
      loop {
        ...
        match current.parent() {
          ...
          _ => return Err(io::Error::new(io::ErrorKind::InvalidInput, "cannot find the logical and physical block size information")),
        }
      }
    },
  }
